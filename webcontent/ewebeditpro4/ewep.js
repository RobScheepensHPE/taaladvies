/* Copyright Ektron, Inc. 07/05/05 */function CEktMonarch(){this.aryAutoInstall=[];this.requestAutoInstall=CEktMonarch_requestAutoInstall;
...
function toLiteral(object){var sLiteral="";switch (typeof object){case"undefined": sLiteral="undefined";
 ...
 sLiteral=sLiteral.replace(/\n/g,'\\n');break;case"object": if (null==object){sLiteral="null";
 } else if ("undefined"==typeof object.length){for (var propName in object){if (sLiteral.length > 0){
 sLiteral +=", ";} sLiteral +="'" + propName +"':" + toLiteral(object[propName]);
...
function PlatformInfo(){var ua=window.navigator.userAgent.toLowerCase();this.isWindows=(ua.indexOf("win") > -1);
 ...
 this.isFirefox=((ua.indexOf("firefox/1.") !=-1) && (!this.isOpera));this.isNetscape60=false;
 var pIE=ua.indexOf("msie ");this.isIE=((pIE > -1) && !this.isOpera);if (this.isFirefox){
 this.browserVersion=7.2;this.isNetscape=true;this.isOpera=false;this.isNetscape60=false;
 } else if (this.isOpera){this.browserVersion=parseFloat(ua.substring(pOpera + 6));
 } else if (this.isIE){this.browserVersion=parseFloat(ua.substring(pIE + 5));} else if (this.isNetscape){
...
function eWebEditProInstance(name,width,height,parameters){if (typeof parameters !="object" || (null==parameters)){
 ...
 this.validateContent=eWebEditProInstance_validateContent;if (typeof cSETMETHOD !="undefined"){
 this.editorSetMethod=cSETMETHOD;} if (!parameters.editorGetMethod){if (typeof cGETMETHOD !="undefined"){
 this.editorGetMethod=cGETMETHOD;} } else{this.editorGetMethod=parameters.editorGetMethod;
 ...
 this.insertFileLink=eWebEditProInstance_insertFileLink;this.insertFileLinkDeferred=eWebEditProInstance_insertFileLinkDeferred;
...
function eWebEditProInstance_linkedElement(name){var objElem=null;var objInfo=null;
 if (name){objInfo=findFormAndElement(name,this.evalWindow);} else{if (this.linkName && !this.elemName && !this.formName){
...
function eWebEditProInstance_insertFileLink(strSrcFileLocation,bLocalFile,strLinkedText){
 setTimeout('eWebEditPro.instances["' + this.name +'"].insertFileLinkDeferred(' + toLiteral(strSrcFileLocation) +', ' + bLocalFile +', ' + toLiteral(strLinkedText) +')',1);
}
...
var cPROGID="eWebEditProLibCtl4.eWebEditPro";var cCLASSID="64A6114F-2976-4634-BE36-134BF84D369C";
var cVERSION="4,3,0,8";var cSETMETHOD="setDocument";var cGETMETHOD="getBodyHTML";
...
eWebEditProAddInstanceType(eWebEditProActiveXType);function eWebEditProActiveXType_refreshStatus(parameters){
 ...
 this.versionInstalled=elementVersionInstalled;} objAXPluginElement=null;} this.isSupported=this.isSupported && this.checkLicense(parameters);
...
function eWebEditProEditor(name,width,height,parameters){if (typeof parameters !="object" || (null==parameters)){
 ...
 } if ((parameters.isLicense("xml",parameters["license"]) || parameters.isLicense("XML",parameters["license"])) && objActiveXElement.isIE && objActiveXElement.browserVersion >=5.0 && s_numInstances==1){
 sHtml +='<object classid="clsid:88d969c0-f192-11d4-a65f-0040963251e5" height="0" width="0" codebase="' + parameters.path +'msxml4.cab#version=4,10,9404,0"></object>';
 } if (typeof cLPKOBJECT=="string" && cLPKOBJECT.length > 0 && objActiveXElement.isIE && s_numInstances==1){
 sHtml +=cLPKOBJECT;} if (!objActiveXElement.isInstalled && !objActiveXElement.isAutoInstallSupported){
...
function eWebEditProEditor_validateContent(){if (!this.editor.CheckContent("whole")){
 this.initEvent("onerror");this.event.reason=this.editor.XMLProcessor().ErrorCode;
 this.event.message=this.editor.XMLProcessor().ErrorReason;return false;} else{return true;
 }}
...
function eWebEditProFactory_create(name,width,height,parameters){if (typeof parameters !="object" || (null==parameters)){
 ...
 bNeedAutoInstall=(this.versionsRequired !=this.autoInstallCookie.getCookie());} if (bNeedAutoInstall){
 if (ektMonarch){this.parameters=parameters;ektMonarch.requestAutoInstall(eWebEditPro);
 ...
 } else if (eWebEditProMessages.clientInstallMessage){if(this.isFirefox){conditional_write('<br><font face="Arial" size="2" color="red"><b>Web Editing for Mozilla <span style="color:#BA150B; font-weight:bold; font-size:larger">Fire</span><span style="color:#DF7419; font-weight:bold; font-size:larger">fox</span>: </b></font>' + eWebEditProMessages.clientInstallMessage,objInstance,parameters);
