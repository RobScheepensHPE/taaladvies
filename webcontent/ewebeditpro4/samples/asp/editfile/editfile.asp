...
<body>
<%
Const cACTIONPAGE = "editfile.asp"
Dim strContent
Dim objFSO
Dim objFile
Dim bMapPath
...
If bSave Then
	...
	Set objFile = objFSO.CreateTextFile(strServerFilename)
...
ElseIf Len(strFilename) > 0 Then
	On Error Resume Next
	Set objFile = objFSO.OpenTextFile(strServerFilename)
	If Err.Number = 0 Then
...
<input type=submit name="btnSave" value="Save" onclick='frmMain.action += "?save"'>
<%
